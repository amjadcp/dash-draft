...
- **Surface-scoped contrast.** "Emphasis" sections (e.g. a CTA block) invert relative to the *current* theme, not to a fixed value — this is implemented by re-scoping the color variables inside a `.surface-emphasis` container rather than hardcoding black/white. See `component-patterns.md` for the exact mechanism.
